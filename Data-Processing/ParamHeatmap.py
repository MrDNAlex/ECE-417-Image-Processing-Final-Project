...
for video in videoNames:
    ...
    # Calculate the global color scale between both algorithms
    globalMin = min(df['SSIM OpenCV AVG'].min(), df['SSIM Otsu AVG'].min())
    globalMax = max(df['SSIM OpenCV AVG'].max(), df['SSIM Otsu AVG'].max())

    # Create the pivot tables
    pivotCv = pd.pivot_table(
        df, values='SSIM OpenCV AVG', index=['A', 'T', 'Resolution'], columns=['K', 'M'], aggfunc='mean'
    ).dropna(how='all').dropna(axis=1, how='all')

    pivotOtsu = pd.pivot_table(
        df, values='SSIM Otsu AVG', index=['A', 'T', 'Resolution'], columns=['K', 'M'], aggfunc='mean'
    ).dropna(how='all').dropna(axis=1, how='all')

    # Create Subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(30, 16), sharey=True)
    fig.suptitle(f"SSIM Average Score Heatmap - {video}", fontsize=26, fontweight='bold', y=0.96)
    ...
    outputImg = os.path.join(outputDir, f"ParamSweep-Heatmap-{video}.png")
    plt.savefig(outputImg)
    plt.close()
    
    print(f"Successfully saved shared hierarchical heatmap for {video} to {outputImg}")