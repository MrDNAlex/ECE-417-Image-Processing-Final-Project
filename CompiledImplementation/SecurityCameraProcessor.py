...
class SecurityCameraProcessor:
    ...
    rawVideo: cv2.VideoWriter
    """Video Writer for the Raw Security Camera Footage"""
    ...
    def run(self):
        ...
        lastFrame = np.zeros((self.height, self.width, 3), dtype=np.uint8)
        ...
        self.comparisonVideo = cv2.VideoWriter(os.path.join(folderPath, f"{self.fileName}-Comparison.mp4"), subtractorVideoSource, FPS, (self.width * 2, self.height * 2))
        self.rawVideo = cv2.VideoWriter(os.path.join(folderPath, f"{self.fileName}-Raw.mp4"), subtractorVideoSource, FPS, (self.width, self.height))
        self.compressedVideo = cv2.VideoWriter(os.path.join(folderPath, f"{self.fileName}-Compressed.mp4"), subtractorVideoSource, FPS, (self.width, self.height))
        ...
        while True:
            ...
            xOffset = int(self.width / 2)
            bottomRow[:, xOffset:xOffset + self.width] = lastFrame
            
            combinedView = np.vstack((topRow, bottomRow))
            ...
            self.rawVideo.write(frame)
            self.compressedVideo.write(lastFrame)
            # Add an if statement with a new toggle setting
